...
resource "null_resource" "update_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      aws eks update-kubeconfig --name ${aws_eks_cluster.this[0].id} --region ${var.region}
    EOF
  }

  triggers = {
    cluster_id = aws_eks_cluster.this[0].id
  }
...
resource "kubernetes_config_map" "aws_auth" {
  ...
  depends_on = [
    # Required for instances where the configmap does not exist
