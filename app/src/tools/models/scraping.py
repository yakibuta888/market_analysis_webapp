...
import pandas as pd
from tools.models.database import db_session
from tools.models.database import engine
from tools.models.database import init_db
from tools.models.models import CrudeOilData, Gold
...
class GetTables(object):
    ...
    def to_database(self, df, asset):
        # asset_db = Data()
        # asset_db.__tablename__ = asset

        # if not db_session.query(CrudeOilData).all():
        # init_db()

        df.to_sql('crude_oil', engine, index=False, if_exists='append')
        # row = database_name(df)
        # db_session.add(row)
        # db_session.commit()
